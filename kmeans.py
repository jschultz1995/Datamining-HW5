...
def distance(instance1, instance2):
    ...
    for i in range(1, len(instance1)):
        if(instance1[i] < instance2[i]):
            #print "Here1 " , i
            min_list.append(instance1[i])
            max_list.append(instance2[i])
        else:
            #print "Here2 ", i
            min_list.append(instance2[i])
            max_list.append(instance1[i])
    sum_min = sum(min_list)
    sum_max = sum(max_list)
    sumOfSquares = (1 - (sum_min/sum_max))**2
    """
    """
    #1-Cosine Similarity
    mod_inst1 = instance1[1:]
    mod_inst2 = instance2[1:]
    cos_sim = dot(mod_inst1, mod_inst2)/(norm(mod_inst1)*norm(mod_inst2))
    sumOfSquares = (1 - cos_sim)**2
    """
    """
    #Euclidean
    sumOfSquares = 0
    ...
    sumOfSquares = 0
    for i in range(1, len(instance1)):
        sumOfSquares += -1 * (instance1[i] - instance2[i])
...
def kmeans(instances, k, animation=False, initCentroids=None):
    ...
    print "Iterations with Manhattan: ", iteration
    print "SSE for Manhattan: ", withinss 
    return result
...
#dataset = loadCSV("iris2.csv")
dataset = [[3,5],[3,4],[2,8],[2,3],[6,2],[6,4],[7,3],[7,4],[8,5],[7,6]]
